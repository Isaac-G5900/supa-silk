...
def create() -> None:
    ...
    # Load any .env values (e.g. OPENAI_API_KEY)
    load_dotenv()
    # openai_key = os.getenv("OPENAI_API_KEY")
    # # if not openai_key:
    #     print("WARNING: OPENAI_API_KEY not set; OpenAI‐backed tools will fail at runtime.")
...
def send_message(resource_id: str, user_id: str, session_id: str, message: str) -> None:
    ...
    for event in remote_app.stream_query(
        ...
        if hasattr(event, "text"):
            ...
            continue
        # 3) Fallback dict parts
        if isinstance(event, dict) and event.get("parts"):
...
def main(argv=None):
    """Main entrypoint for deploy-remote."""
    if argv is None:
        argv = flags.FLAGS(sys.argv)
    else:
        argv = flags.FLAGS(argv)

    # Load .env for GCP creds
    load_dotenv()
